...
test('人工金额输入保留单位与计价口径，交由同一后端解析器处理',async t=>{
  ...
  assert.equal(saved.basis,'contract')
  assert.equal(saved.package,'1')
})

test('修改人工签章内容后撤销旧确认，签字与盖章状态独立', async t => {
  ...
  const saved=await page.evaluate(()=>window.saved)
  assert.equal(saved.signature_manually_confirmed,false)
  assert.equal(saved.signature_status,'pending')
  assert.equal(saved.seal_status,'pass')
  const confirmation=page.locator('.manual-review-field').filter({has:page.getByText('已对照原件确认全部所需签字',{exact:true})})
  await confirmation.getByRole('button',{name:'修改',exact:true}).click()
  await confirmation.getByRole('checkbox').check()
  assert.equal(await page.evaluate(()=>window.saved.signature_manually_confirmed),true)
})
